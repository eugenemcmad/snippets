...
// 1000000	1216 ns/op
func BenchmarkRxReplaceAllString(b *testing.B) {
...
// 1000000	1206 ns/op
func BenchmarkRxReplaceAll(b *testing.B) {
	var rx = regexp.MustCompile(`\$\{([\w]{1,1000})\}`)
...
// 10000000	135 ns/op
func BenchmarkBytesPreReplace(b *testing.B) {
	z := []byte(`azaza ${Name} zazaz. `)
	k := `Name`
	v := []byte(`Mary`)
	for i := 0; i < b.N; i++ {
...
func BenchmarkRxRestReplaceAllString(b *testing.B) {
	z := `azaza ${Name} zazaz. `
...
// 1000000	1210 ns/op
// alexflint/go-restructure/regex
func BenchmarkRxRestReplaceAll(b *testing.B) {
	z := []byte(`azaza ${Name} zazaz. `)
	v := []byte(`Mary`)
	rx := regex.MustCompile(`\$\{([\w]{1,1000})\}`)
	for i := 0; i < b.N; i++ {
...
// 500000	2799 ns/op
// dlclark/regexp2
func BenchmarkRx2Replace(b *testing.B) {
	z := `azaza ${Name} zazaz. `
